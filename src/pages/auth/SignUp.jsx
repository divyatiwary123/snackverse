...
export default function SignUp(){
  const [name,setName]=useState(''); const [email,setEmail]=useState(''); const [password,setPassword]=useState(''); const [error,setError]=useState(null)
  const { signUp } = useContext(AuthContext); const navigate = useNavigate()
  async function submit(e){ e.preventDefault(); setError(null); try{ await signUp({ name,email,password }); navigate('/') }catch(err){ setError(err.message) } }
  return (
    <main className="max-w-md mx-auto p-6">
      ...
      <form onSubmit={submit} className="mt-4 flex flex-col gap-3 bg-white p-4 rounded shadow">
        ...
        <button className="px-4 py-2 bg-amber-600 text-white rounded">Sign up</button>
        <div className="text-sm text-gray-600">Already have an account? <Link to="/signin" className="text-amber-600">Sign in</Link></div>
      </form>
    </main>
  )
}
